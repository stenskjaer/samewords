...
class TextSegment(list):
    """
    A chunk of text. Can be anything from a whole paragraph to a short
    expression. Contains list of normal string and CritText objects.
    """
    ...
    def content_split(self,
        ...
        """
        Split the input `search_string` into list demarcated by `\edtext{
        }`-items.

        :param search_string: The input string.
        :param return_list: The list of text objects that should be returned.
        :param first: Indicate whether this is the first iteration, used in
        ...
        if r'\edtext' in search_string:
            ...
            maintext_note_len = len(
                Macro(search_string[appnote_start:]).complete_macro())
            crit_note_len = Brackets(
                search_string,
                start=appnote_start + maintext_note_len).length
            appnote_end = appnote_start + maintext_note_len + crit_note_len

            if appnote_start > 0:
                return_list.append(search_string[:appnote_start])
            return_list.append(
                CritText(search_string[appnote_start:appnote_end]))
            return self.content_split(
                search_string[appnote_end:], return_list, first=False)
...
class CritText(str):
    ...
    It takes a text segment containing exactly one critical text node like
    so: "\edtext{ maintext}{critical text}". If the second argument contains
    a `\lemma`, that is used for determining the search words of the objects.
    """
    ...
    def __init__(self, input_string: str) -> None:
        ...
        self.critical_note = Brackets(
            self.content,
            start=Brackets(self.content, macro=r'\edtext').length).content
        self.has_lemma = True if self.critical_note.find(
            r'\lemma') is not -1 else False
    ...
    def define_search_words(self) -> List[str]:
        """
        From the lemma content, define which type of lemma we have and return
        appropriate list.
        ...
        if self.has_lemma:
            ...
            ellipsis_pattern = re.compile(r'(\\l?dots({})?)' + add_ellipses(
                settings.ellipsis_patterns))
            if re.search(ellipsis_pattern, self.lemma_content):
                # Covers lemma with ellipsis_pattern.
                ...
                # Covers multiword lemma. We join them to one "search_word"
                # as we look for that specific phrase in proximity, not just
                # any of its containing words.
    ...
    def replace_in_maintext_note(self,
                                 replace_word: str,
                                 replace_string: str = None,
                                 lemma_level: int = 1) -> str:
        """
        Replace all instances of `replace_word` in a apparatus note (full
        `\edtext{}{}`) and return the updated string.

        We instantiate self.maintext_note or `replace_string` (if given) as
        TextSegment, thus splitting it into segments of critical notes and
        regular text. That list is iterated, wrapping any match of
        `replace_word` as we go along.

        Updates self.maintext_note and return the updated self.content or
        returns the updated `replace_string` if given.

        The possibility of passing `replace_string` is required for recursive
        calls in nested edtext elements.

        :param replace_word: The word to be replaced.
        :param replace_string: A string to wrap replacement words (optional).
        :param lemma_level: The level from which the lemma refers to the word to
            be `replace_word`.
        ...
        if self.dotted_lemma:
            # We have a ldots lemma, so only replace at the beginning and end
            #  of the edtext element.
            ...
            for pivot_index, edtext_content in enumerate(edtext_content_list):
                ...
                if isinstance(edtext_content, CritText):
                    ...
                    edtext_content_list[pivot_index] = self.assemble(
                        maintext=maintext,
                        critical=edtext_content.critical_note)
        ...
        if replace_string is not None:
            ...
            # Update the maintext parameter and update self.content with
            # assemble()
            self.maintext_note = return_string
            return self.assemble()

    def replace_last_maintext_word(self,
                                   replace_string: str,
                                   replace_word: str,
                                   lemma_level: int = 1) -> str:
        """
        Replace only on the very last maintext word of `replace_string` (if
        it matches).
        ...
        if isinstance(segment, CritText):
            maintext = self.replace_last_maintext_word(
                segment.maintext_note, replace_word, lemma_level)
            segments[-1] = segment.assemble(maintext=maintext)
        else:
            previous = ' '.join(segment.split(' ')[:-1])
            last = segment.split(' ')[-1]
            updated_last = self.replace_in_maintext_note(replace_word, last,
                                                         lemma_level)
            segments[-1] = previous + ' ' + updated_last

        return segments.to_string()

    def replace_first_maintext_word(self,
                                    replace_string: str,
                                    replace_word: str,
                                    lemma_level: int = 1) -> str:
        """
        Replace only the very first maintext word of `replace_string` (if it
        matches).
        """
        segments = TextSegment(replace_string)
        self.dotted_lemma = False
        segment = segments[0]
        if isinstance(segment, CritText):
            maintext = self.replace_first_maintext_word(
                segment.maintext_note, replace_word, lemma_level)
            segments[0] = segment.assemble(maintext=maintext)
        else:
            first = segment.split(' ')[0]
            rest = ' '.join(segment.split(' ')[1:])
            updated_first = self.replace_in_maintext_note(replace_word, first,
                                                          lemma_level)
    ...
    def replace_in_critical_note(self, replace_word: str) -> str:
        ...
        return_string += replace_in_string(
            replace_word, lemma_content, lemma_level=0)
        position += len(lemma_content)

        # Add the following app note, where nothing should be changed,
        # and return it
        return_string += self.critical_note[position:]
    ...
    def replace_in_edtext_args(self, replace_word: str,
                               lemma_level: int = 1) -> str:
...
class Context:
    ...
    def update(self,
               raw_before: TextSegment,
               raw_after: TextSegment,
               lemma_level: int = 1):
        """
        Build the surrounding context. If is a list of lists, list 0 contains
        context on lvl 1, list 1 contains context on lvl 2 etc.
        """
        prox_context_before = self.iter_proximate_words(
            raw_before, side='left')
    ...
    def iter_proximate_words(self,
                             ...
                             length=15):
        """
        Get a suitable amount of items from input_list to serve the amount of
        proximity words defined by length.

        When the `side` is `left`, it is reversed before and after processing
        to start the counting from the end.

        :param input_list: The list to pull from
        :param index: The index from where the search should start.
        :param word_count_sum: Accumulative word counter for finding the right
            size chunk.
        :param side: Indicate whether we are searching left of the pivot. In
            that case, we reverse the list too.
        :param length: Amount of words required for the chunk.
        :return: List of at least length equal to the word count defined by
            `length` on each side of the pivot word (unless we are at the fringe
            of a string).
        """
        ...
        if word_count_sum < length and index + 1 < len(input_list):
            return self.iter_proximate_words(input_list, index + 1,
                                             word_count_sum, side)
        else:
            if side == 'left':
    ...
    def content(search_string: str, position: int, macro: str) -> str:
        ...
        if macro:
            if search_string[position:position + len(macro)] == macro:
                ...
                raise ValueError(
                    "The indicated macro, %s, did not occur at position %i."
                    "Context: %s" %
        ...
        if search_string[position] == '{':
            ...
            raise ValueError(
                "The first symbol of the string must be an opening bracket "
        ...
        while stack > 0:
            try:
                ...
                if symbol == '{':
                    ...
                    stack -= 1
                elif symbol == '\\' and search_string[position + 1] in escapes:
                    capture += symbol
                ...
                raise ValueError(
                    "Unbalanced brackets. The provided string terminated "
                    "before all brackets were closed.")
        return capture[:-1]


class Macro:
    """A full latex macro, identified from `start` to where the macro is
    completed.
    """
    ...
    def before_opening(self) -> str:
        ...
        if opening:
            ...
            if re.search(r'\s', self.input_string[self.start:]):
                space_pos = re.search(r'\s',
                                      self.input_string[self.start:]).start()
            return self.input_string[self.start:space_pos]

    def complete_macro(self) -> str:
        if self.has_opening:
            pos = self.start + self.input_string[self.start:].find('{')
            return self.before_opening \
                   + '{' + Brackets(self.input_string, start=pos).content + '}'
        else:
            return self.before_opening
...
class Word(str):
    ...
    def __iadd__(self, other):
        """Perform inplace addition by copying the current object to new and
        making addition. """
        old = self
        w = Word(self + other)
        w.start = old.start
    ...
    def flatten(self, obj) -> Iterable[str]:
        """Return a flat list from an arbitrarily nested list of Word
        objects."""
    ...
    def clean(self, search_string: str, global_offset: int = 0):
        ...
        while position < len(search_string):
            ...
            if re.match('\w', search_string[position]):
                # If we match word character, store that word and start
            ...
            if search_string[position] == '\\':
                ...
                if macro.name in ignored_macros:
                    ...
                    offset_pos = position + global_offset + len(
                        macro.before_opening + '{')
                    word = Word(
                        self.clean(macro.content(), global_offset=offset_pos))
                    ...
                    if word:
                        # If we are recording a word, store content of macro
                        # as part of word.
                        position += len(macro.complete_macro())
                        word += macro.content()
                        ls[-1] = word
...
def search_in_proximity(search_word: str,
                        context_before: List[List[str]],
                        context_after: List[List[str]]) -> bool:
    """
    Check whether a search word is present in the one of two lists. The two
    input lists contain the context of the search word.

    We need to search for a regex demarcating the string with boundaries to
    make sure we don't just match a partial word ("so" matching "something").
    We can't demarcate by whitespace, as that may not always be present.
...
def replace_in_proximity(context_before_list: ContextList,
                         context_after_list: ContextList,
                         search_word: str) -> Tuple[ContextList, ContextList]:
    """
    Replace a specified search word in two lists containing the text
    surrounding a chunk of text.

    This is used for replacing a word that is present in an `\edtext{
    }`-element and that is confirmed to be in the proximate context.
    ...
    :param context_before_list: The context list contaning the next content.
    :return: The two updated lists where samewords of search word are marked.
    """

    def replace_in_context_list(context_list: ContextList,
                                word: str) -> ContextList:
...
def replace_in_string(replace_word: str,
                      replace_string: str,
                      lemma_level: int = 0) -> str:
    """
    Recursively replace the search word in the search string with a version
    that is wrapped in sameword. This is designed to handle replacement of
    multiword phrases in the string too. The wrapping is done intelligently
    so that what is already wrapped is not double wrapped, and lemma levels
    are incorporated.
    ...
    def check_list_match(pattern_list: List[str],
                         replace_list: List[Word],
                         return_list: List[str] = list(),
                         first: bool = True) -> List[Word]:
        """Check whether a word string in `pattern_list` (search phrase) is
        matched in `replace_list`.

        Check whether first item of `pattern_list` is contained in first item
        of `replace_list`. If it does, move on to next item in `pattern_list`
        until it is exhausted. This way it matches whole string of words
        specified in `pattern_list` in `replace_list`.

        When the caller of this function iterates through the `replace_list`,
        it is incrementally checked in full. We want to keep the formatting
        of the search list, so items from that are returned in order as they
        are.

        During matching we need to maintain whitespace in the list being
        matched (`replace_list`). So when there is any content in the
        return_list (a match has started), and a match fails because the
        replace_list content is whitespace, add that to the return_list.

        :param pattern_list: List of search words to be matched in `replace_list`.
        :param replace_list: List of words to be matched.
        :param return_list: The results to be returned, built successively in
            recursive calls.
        :param first: Indicate whether we are at the first iteration on a
            recursive stack.
        :return List of items from `replace_list` that match items in
            `pattern_list`.
        ...
        try:
            word = replace_list[0]
            # If first item of match list is in word item (first item in
            # sublist) of replace_list, register the start and end of that
            # word in return_list.
    ...
    def make_replacements(search_list: List[str],
                          word_list: List[Word],
                          orig_string: str,
                          wrap_item: Word = None,
                          context: List[Word] = None,
                          offset: int = 0) -> str:
        """
        Directing function for replacement.

        This analyses a string of `Word` objects (returned by the `Words(
        ).list` parameter). Since the clean function of `Words` returns a
        potentially nested list (sameword annotations are processed as a
        nested list of their content), this handles `Word().content` as
        either `str` or `list`.

        :param search_list: List of `str` objects providing match patterns for
            string matching.
        :param word_list: List of `Word` objects representing a cleaned version
            of `orig_string`.
        :param orig_string: The original form of the string with the content we
            want to replace.
        ...
        :param offset: Used for updated position index on multiple matches on
            same string.
        :return: Updated string containing sameword annotations.
        """
        if not word_list:
            # Word list is empty and we made no matches. Return the original
            # string.
            return orig_string
        if isinstance(word_list[0].content, str):
            ...
            if matches:
                ...
                wrapped_phrase = wrap_phrase(
                    phrase, lemma_level=lemma_level, wrap=wrap_item)
                ...
                return make_replacements(
                    search_list, word_list[1:], updated, offset=offset)
            else:
                if context:
                    # No match on this recursion level, so we continue from
                    # parent context.
                    return make_replacements(
                        search_list,
                        context[1:],
            ...
            # The word contains a list of Word obj's that we must go through.
            #  Replace with content as input list, the object as position
            # context and the whole word_list for context preservation when
            # we continue after the recursion.
            return make_replacements(
                search_list,
                word_list[0].content,
                orig_string,
                wrap_item=word_list[0],
                context=word_list)
    ...
    if words_list:
        updated_replace_string = make_replacements(search_words, words_list,
                                                   replace_string)
    else:
...
def wrap_phrase(phrase: str, lemma_level: int = 0, wrap: Word = None) -> str:
    ...
    This means that if the word or phrase is already wrapped in a \sameword{
    }, it cannot be wrapped in another. If one or more words of a phrase are
    wrapped, it should wrap the whole phrase. It also needs to handle the
    ...
    if sameword_wrap:
        if lvl_match:
            ...
            if this_level:
                # If we have a lemma level here, combine with existing level
                # argument.
                lvl_set = set([int(i)
                               for i in lvl_match.split(',')] + [this_level])
...
def critical_note_match_replace_samewords(text: str) -> str:
    ...
    The included function `sub_processing` orchestrates the main process. The
    wrapper function passes a TextSegment object to the processing function.
    The TextSegment class segments the string into chunks of `\edtext{
    }`-elements (stored as CritText objects) and regular text. All CritText
    objects are processed by identifying whether any search words of the
    content in the critical note is present in the proximate context. When
    that is the case, we process the context along with the critical note
    itself. If the critical note contains any nested `\edtext{}` elements,
    we recurse to the innermost element and start from there.

    Search and replace in proximity works like this: Receive a list of lists.
    Each list in the list represents a lemma level: List 1 is the context of
    first edtext element, list 2 is the context of the contained edtext
    element etc.
    ...
    def sub_processing(text_object, context=Context(), lemma_level=1):
        ...
        for pivot_index, segment in enumerate(text_object):
            ...
            if isinstance(segment, CritText):
                ...
                # Build the surrounding context. If is a list of lists. List
                # 0 contains context on lvl 1, list 1 contains context on lvl
                #  2 etc.
                context.update(text_object[:pivot_index],
                               text_object[pivot_index + 1:], lemma_level)

                if r'\edtext' in segment.maintext_note:
                    # We have a nested apparatus note here, so before moving
                    # on, we process that.
                    lemma_level += 1
                    output = sub_processing(
                        TextSegment(segment.maintext_note),
                        context,
                        lemma_level=lemma_level)
